<?php
	...
	if(isset($_POST['btnedit'])) {
		...
		if (!empty($_POST['itemName']) && !empty($_POST['itemPrice']) ) {
			...
			$itemPrecioOferta = $sqlconnection->real_escape_string($_POST['itemprecioOferta']);

			$updateItemQuery = "UPDATE tbl_menuitem SET menuItemName = '{$itemName}' , price = {$itemPrice}, precioOferta = {$itemPrecioOferta} WHERE menuID = {$menuID} AND itemID = {$itemID} ";
